...
export enum BuyInRequestStatus {
  PENDING = 'pending',
  APPROVED = 'approved',
  REJECTED = 'rejected'
}

@Entity({ name: 'buyin_requests' })
@Index(['club', 'status'])
@Index(['player', 'status'])
export class BuyInRequest {
  ...
  @Column({ name: 'requested_amount', type: 'decimal', precision: 10, scale: 2 })
  requestedAmount!: number;

  @Column({ name: 'current_table_balance', type: 'decimal', precision: 10, scale: 2, nullable: true })
  currentTableBalance!: number | null;

  @Column({ type: 'varchar', length: 20, default: BuyInRequestStatus.PENDING })
